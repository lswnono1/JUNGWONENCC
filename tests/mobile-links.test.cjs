...
test('new loader includes nine layers and current version',()=>{const text=fs.readFileSync(path.join(root,'lawmonitor/app.js'),'utf8');assert.match(text,/length:9/);assert.match(text,/1\.5\.2/)});
test('service worker caches new layer and old entry point',()=>{const text=fs.readFileSync(path.join(root,'lawmonitor/sw.js'),'utf8');assert.match(text,/length:9/);assert.match(text,/v1\.5\.2/);assert.match(text,/app\.js\?v=1\.4\.0/);assert.match(text,/key\.startsWith\(CACHE_PREFIX\)/)});
