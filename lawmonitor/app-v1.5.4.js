/* Jungwon Law Monitor 1.5.4: immutable full application */
(function(){
'use strict';
...
async function fetchAppendices(item) {
  ...
  for (let page = 1; page <= 30; page += 1) {
    ...
    const result = v154SearchResult(payload,['별표명','별표서식명','별표일련번호','별표서식일련번호']);
...
async function syncNow(options = {}) {
  ...
  if (!state.secrets?.lawOc) return toast('설정에서 API OC를 입력하세요.');
  state.syncing = true;
  const items = (await managedRows()).filter((row) => row.enabled && (!options.failedOnly || /실패|미확인|부분|중계/.test(row.checkStatus || '미확인')));
  if (!items.length) { state.syncing = false; return toast(options.failedOnly ? '재조회할 실패 항목이 없습니다.' : '활성 관리대상이 없습니다.'); }
  ...
  try {
    for (let index = 0; index < items.length; index += 1) {
      ...
      if (revisionsOk) try {
        ...
        if (appendices.length && revisions.length) {
          ...
          for (const appendix of appendices) {
            const eventKey = byDate.get(appendix.revisionDate);
      ...
      const latest = revisionsOk ? revisions[0] || {} : {};
      const checkStatus = [lawStatus === '정상' ? '개정이력 정상' : lawStatus, appendixStatus === '정상' ? '별표 정상' : appendixStatus === '미확인' ? '별표 미확인' : appendixStatus, noticeStatus === '정상' ? '' : noticeStatus].filter(Boolean).join(' · ');
      await dbPut('managed',{
        ...item,
        ...
        lastNoticeCheckedAt:noticeStatus === '정상' ? new Date().toISOString() : item.lastNoticeCheckedAt || '',
        checkStatus,
    ...
    await addLog(lawFailures || appendixFailures || noticeFailures || relayError ? '부분완료':'완료',`관리대상 ${items.length}건 · 신규 개정 ${newChanges}건 · 신규 입법예고 ${newNotices}건 · 개정이력 실패 ${lawFailures}건 · 별표 실패 ${appendixFailures}건 · 입법예고 실패 ${noticeFailures}건`);
    toast(lawFailures || appendixFailures || noticeFailures || relayError ? `부분 완료 · 이력 실패 ${lawFailures}건 · 별표 실패 ${appendixFailures}건 · 예고 ${relayError ? '미확인' : noticeFailures+'건 실패'}` : `동기화 완료 · 신규 개정 ${newChanges}건 · 신규 입법예고 ${newNotices}건`,5000);
  } finally {
    state.syncing = false;
    renderSync(false,'',0);
    await refreshAll();
